...
function bash-restore(){
    bash-readsettings
        echo "Is this a Windows bash shell or Ubuntu? (enter w/u)";
        read shellenv;
          if [ "$shellenv" == "w" ]; then
            shellenv="windows"
...
index=${#prefix}
    ...
    if [ "$shellenv" == "windows" ]; then # aimed at the ming64 shell for windows which does not have functions such as sudo
...
function bash-writesettings() {
    echo "$environment,$www_sitefocus,$ssh1,$ssh2,$wwwsite1,$wwwsite2,$wwwsite3,$gituname,$phpNo,$ipgateway,$welcomemsg,$wwwroot,$shellenv" >~/.bash_cfg
}
...
function www-showcfg() {
    echo "Current Platform: "$shellenv
...
function echo-h1() {
    textoutput=$1
    if [ "$shellenv" == "windows" ]; then # assume we are using the gitbash ming shell so sudo does not exist
    echo $textoutput
  else
    figlet $textoutput
    fi
